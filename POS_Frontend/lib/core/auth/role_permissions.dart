...
class RolePermissions {
  ...
  static const _ventes = {1, 2, 3, 5, 6};
  ...
  static const _commandesAttente = {1, 2, 3};
  ...
  static bool canAccess(AppModule module, Utilisateur? user) {
    ...
    final allowed = switch (module) {
      ...
      AppModule.commandesAttente => _commandesAttente,
    };
    return allowed.contains(roleId) || _fallbackByNomRole(module, user);
  }

  /// Ouverture / fermeture de caisse — caissier ou administrateur uniquement.
  static bool canOperateCaisse(Utilisateur? user) {
    if (user == null) return false;
    return user.isAdministrateur || user.isCaissier;
  }

  /// Encaissement final (paiement) — pas le serveur.
  static bool canEncaisserVente(Utilisateur? user) {
    if (user == null) return false;
    return user.isAdministrateur || user.isGerant || user.isCaissier;
  }

  /// Prise de commande transmise au caissier.
  static bool canEnvoyerCommande(Utilisateur? user) {
    if (user == null) return false;
    // Explicite : ne pas confondre avec les autres rôles POS.
    if (user.isAdministrateur || user.isGerant || user.isCaissier) {
      return false;
    }
    return user.isServeur;
  }

  /// Liste des commandes en attente (caissier / admin / gérant).
  static bool canGererCommandesEnAttente(Utilisateur? user) {
    return canEncaisserVente(user);
  }

  /// Modification des paramètres système / sauvegardes — administrateur uniquement.
  static bool canManageParametresSysteme(Utilisateur? user) {
    return user?.isAdministrateur == true;
  }

  static bool canWrite(AppModule module, Utilisateur? user) {
    if (user == null) return false;
    if (module == AppModule.parametres) {
      return canManageParametresSysteme(user);
    }
    if (module == AppModule.dashboard ||
        module == AppModule.pos ||
        module == AppModule.ventes ||
        module == AppModule.rapports ||
        module == AppModule.commandesAttente) {
  ...
  static List<({String label, String route})> itemsMenuLateral(
    ...
    const candidats = <(String, String, AppModule?)>[
      ...
      ('Commandes en attente', AppRoutes.commandesAttente, AppModule.commandesAttente),
      ('Ventes', AppRoutes.ventes, AppModule.ventes),
    ...
    return candidats
        .where((item) => item.$3 == null || canAccess(item.$3!, user))
  ...
  static AppModule _moduleFromRoute(String route) {
    ...
    return switch (route) {
      ...
      AppRoutes.ventes => AppModule.ventes,
      AppRoutes.commandesAttente => AppModule.commandesAttente,
  ...
  static bool _fallbackByNomRole(AppModule module, Utilisateur user) {
    return switch (module) {
      ...
      AppModule.commandesAttente =>
        user.isAdministrateur || user.isGerant || user.isCaissier,
    };
  }
