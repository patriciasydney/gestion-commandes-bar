...
def rapport_ventes(request):
    ...
    par_jour = (
        qs.extra(select={'jour': "date(date_vente)"})
        .values('jour')
    ...
    return Response({
        'periode': {'debut': str(debut) if debut else None, 'fin': str(fin) if fin else None},
        'total_ventes': total,
...
def rapport_produits(request):
    ...
    top_produits = (
        ...
        .annotate(quantite_totale=Sum('quantite'), chiffre_affaires=Sum('sous_total'))
...
def rapport_depenses(request):
    ...
    par_categorie = qs.values('categorie').annotate(total_categorie=Sum('montant')).order_by('-total_categorie')
