...
class AchatSerializer(serializers.ModelSerializer):
    """Lecture d'un achat avec ses lignes."""
    details = DetailAchatSerializer(many=True, read_only=True)

    class Meta:
        model = Achat
        fields = [
            'id_achat', 'date_achat', 'montant_total', 'statut',
            'fournisseur', 'utilisateur', 'details',
        ]
...
class AchatCreateSerializer(serializers.ModelSerializer):
    """
    Enregistrement d'une réception de livraison :
    - crée l'achat + les lignes
    - incrémente le stock de chaque produit reçu (verrouillage FOR UPDATE)
    - historise chaque mouvement dans mouvements_stock
    Le tout dans une seule transaction (cf. .cursorrules).
    """
    ...
    def create(self, validated_data):
        ...
        for line in details_data:
            ...
            DetailAchat.objects.create(
                achat=achat, produit=produit,
                quantite=quantite, prix_unitaire=prix_unitaire,
            )

            # Le stock doit déjà exister (créé lors de l'ajout du produit
            # par le module Gestion commerciale). On le verrouille pour
            # éviter les écarts en cas de réceptions concurrentes.
...
class AchatAnnulationSerializer(serializers.Serializer):
    """Annule un achat et retire le stock qui avait été ajouté."""
